...
# ===================== 頁面設定 =====================
st.set_page_config(page_title="HKJC 落飛分析 (穩定版)", layout="wide")

st.title("🏇 HKJC 落飛分析 (穩定版)")
st.caption("如遇自動抓取失敗，請使用「手動輸入模式」")

# ===================== 側邊欄設定 =====================
st.sidebar.header("⚙️ 設定")

# 選擇模式：自動 vs 手動
mode = st.sidebar.radio("數據來源模式", ["自動抓取 (Auto)", "手動貼上 (Manual)"], index=0)
...
if mode == "自動抓取 (Auto)":
    if st.button("🔄 嘗試抓取數據"):
        with st.spinner("嘗試連線至馬會..."):
            # 這裡保留你之前的 XML 抓取邏輯，當作備用
            try:
    ...
    raw_input = st.text_area("在此貼上數據內容 (JSON):", height=200)
    
    if raw_input:
        df_now = parse_hkjc_data(raw_input, "json")
        if df_now.empty:
            # 如果 JSON 解析失敗，試試 XML 格式 (有些瀏覽器會自動轉 XML)
            df_now = parse_hkjc_data(raw_input, "xml")
            
        if not df_now.empty:
            st.success(f"成功解析 {len(df_now)} 筆數據！")
        else:
            st.error("無法解析內容，請確保複製了完整的 JSON 文字。")
