...
def fetch_hkjc_data(race_no):
    ...
    try:
        ...
        try:
            ...
            return None, "API 回傳非 JSON 格式"

        # 4. 提取數據 (分步安全檢查)
        if data is None:
            return None, "API 回傳空值"
            
        if "OUT" not in 
            return None, "無賠率數據 (可能是日期錯誤或未開售)"
            
        raw_str = data["OUT"]
        if not raw_str:
            return None, "賠率數據為空"

        # 解析 OUT 字串: "1=2.5;2=3.0;..."
        parts = raw_str.split(";")
        odds_list = []
        for p in parts:
            if "=" in p:
                kv = p.split("=")
                if len(kv) == 2 and kv[0].strip().isdigit():
                    try:
                        k = int(kv[0])
                        v = float(kv[1])
                        # 999 通常代表退賽或異常
                        real_val = v if v < 900 else 0.0
                        odds_list.append({"馬號": k, "現價": real_val})
                    except ValueError:
                        continue
...
if app_mode == "📡 實時 (Live)":
            ...
            with st.spinner("正在連接 HKJC 伺服器..."):
                ...
                if df_api is not None:
                    ...
                    if not current_race["current_df"].empty:
                        ...
                        # 先刪除 API DataFrame 裡的馬名 (因為那是預設的 "馬匹 N")
                        df_api = df_api.drop(columns=["馬名"], errors="ignore")
                        # 合併
                        df_api = df_api.merge(old_info, on="馬號", how="left")
                        # 填充空值
                        if "馬名" in df_api.columns:
                            df_api["馬名"] = df_api["馬名"].fillna(df_api["馬號"].apply(lambda x: f"馬匹 {x}"))
                        else:
                            df_api["馬名"] = df_api["馬號"].apply(lambda x: f"馬匹 {x}")
    ...
    with c2:
        st.info(f"上次更新: {current_race['last_update']}")
                ...
                if new_info:
                    ...
                    if not df_info.empty:
                        if not current_race["current_df"].empty:
                            ...
                            # 移除舊列以避免重複
                            cols = [c for c in ["騎師", "練馬師"] if c in df_curr.columns]
...
# ============= History / Overview =============
    ...
    if today_str in history_db:
        ...
        st.info("今日尚未封存數據 (請到 Live 模式按「封存」)")
