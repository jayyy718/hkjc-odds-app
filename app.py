...
# [優化 1] 使用 cache 載入靜態資源，避免每次刷新都重跑
@st.cache_resource
def get_static_resources():
    # Regex 預編譯
    r_int = re.compile(r'^\d+$')
    r_float = re.compile(r'\d+\.?\d*')
    r_chn = re.compile(r'[\u4e00-\u9fa5]+')
    return r_int, r_float, r_chn
...
st.markdown("> 結合即時賠率走勢、騎師與練馬師能力評分，為每匹馬生成 AI 得分，協助實戰玩家快速鎖定值得留意的馬匹。")
...
with st.sidebar:
    ...
    top_pick_threshold = st.slider(
        "TOP PICKS 最低得分",
        min_value=50, max_value=85, value=65, step=1,
...
if app_mode == "📡 實時 (Live)":
    ...
    if not current_race["current_df"].empty:
        ...
        with tab1:
            ...
            with c1: st.metric("最高評分馬匹", f"#{max_horse['馬號']} {max_horse['馬名']}", f"{max_horse['得分']} 分")
            with c2: st.metric("全場平均評分", f"{avg_score} 分")
            with c3: st.metric("落飛馬匹數量", int(drop_count))
                    ...
                    if idx < len(top_picks):
                        ...
                        with col:
                            ...
                            <div class="horse-card top-pick-card">
                                ...
                                <div style="font-size:12px; color:#777; margin-top:-5px; margin-bottom:4px;">{tag_desc}</div>
...
# ============= 歷史模式 =============
    ...
    if 'selected_date' in locals() and selected_date and history_db and str(selected_history_race) in history_db[selected_date]:
        ...
        c1, c2 = st.columns(2)
        with c1: st.metric("最高評分馬匹", f"#{max_h['馬號']} {max_h['馬名']}", f"{max_h['得分']} 分")
        with c2: st.metric("全場平均評分", f"{avg_s} 分")
                ...
                if idx < len(top_picks):
                    ...
                    with col:
                        ...
                        <div class="horse-card" style="background-color:#f9f9f9; border-top: 4px solid #555;">
                            <div style="font-weight:bold; color:#333;">#{row['馬號']} {row['馬名']} (信心 {row['信心級別']})</div>
...
# ============= 今日總覽模式 =============
    ...
    if today_str in history_db:
        ...
        if rows:
            ...
            st.dataframe(overview_df, use_container_width=True, hide_index=True, column_config={
